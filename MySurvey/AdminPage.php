...
class AdminPage extends lib\HomePage {
	...
	protected function init(){
		...
		if(isset($_POST["result"])){
				header("Location: index.php?p=result");
				$result=$_POST['result'];
...
<button class='btn btn-info' type='submit' name='result' value='$row[id]'><span class='glyphicon glyphicon-stats'></span> Auswerten</button>
</form>
...
<td></td>
<td>
<input name="newsurvey" type="text" size="25" maxlength="90" placeholder="Frage"/>
</td>';
for($i=1; $i<5; $i++) {
            $ret.=" <td><input type='text' name='option" . $i . "' size='15' maxlength='90' placeholder='Antwort'></td>";
